...
public abstract class AbstractCrudAndSearchControllerTest<Entity extends Identifiable, SearchCriteria>
	...
	mockMvc.perform(
		...
		.andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(entities.size())));
	...
	assertTrue(secondPageSearchResult != null && secondPageSearchResult.size() == searchResultSize);
	...
	when(searchService.searchEntities(page, size, searchCriteria)).thenReturn(entities.subList(page - 1, size));
