...
public class AbstractEntityQuoteCrudControllerTest<Entity extends QuoteOwner> {
    ...
    public AbstractEntityQuoteCrudControllerTest(String baseUrl, Class<Entity> entityClazz) {
	...
	if (proper) {
	    matcher = status().isOk();
	    when(crudService.createQuote(ArgumentMatchers.anyLong(), ArgumentMatchers.any(Quote.class))).thenReturn(2l);
	} else {
